...
reproduce_dash_c() {
  ...
  echo "out1: $cmd $file $args"
  eval "      $cmd $file $args"
  objdump -d -j .text tmp.ll > out1.obj

  echo "out2: $cmd $file -g $args"
  eval "      $cmd $file -g $args"
  objdump -d -j .text tmp.ll > out2.obj
  diff=`colordiff -u out1.obj out2.obj`

  if [[ $diff == "" ]]; then
    echo "no diff found with .text"
  else
    echo "$diff"
    debug_dash_c "$file" "$args"
  fi
}

debug_dash_c() {
  echo -e "\n### reproduce with clang++ -c debug ###"
  file=$1
  args=$2

  opt0="-mllvm -opt-bisect-limit=0"
  args_nodbg="$args $opt0"
  args_dbg="-g $args $opt0"

  cmd="clang++"


  echo "out1: $cmd $file $args_nodbg"
  eval "$cmd $file $args_nodbg"
  objdump -d tmp.ll > out1.obj

  echo "out2: $cmd $file $args_dbg"
  eval "$cmd $file $args_dbg"
  objdump -d tmp.ll > out2.obj
  diff=`colordiff -u out1.obj out2.obj`
...
main_function () {
  ...
  echo "file: $file"
  echo "args: $args"
  reproduce "$cmd" "$file" "$args"

  reproduce_dash_c "$file" "$args"
...
main ()
{
  cmd="/data/proj/compiler/llvm/llvm-project/clang/utils/check_cfc/clang++ /data/proj/compiler/llvm/test-suite/Bitcode/Benchmarks/Halide/bilateral_grid/driver.cpp -w -c -O1 -o tmp.ll"
  main_function $cmd
